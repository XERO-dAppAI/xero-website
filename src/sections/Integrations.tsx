      ...
      {[...Array(3)].map((_, setIndex) => (
        <div key={setIndex} className="space-y-4">
          {integrations.slice(startIndex, endIndex).map((integration, index) => (
            <motion.div
              key={`${setIndex}-${index}`}
              className="bg-[#2D2654] p-6 rounded-2xl group hover:bg-[#3D3666] transition-colors"
            >
              <div className="bg-[#3D3666] p-3 rounded-xl w-fit mb-4 group-hover:bg-[#4D4676] transition-colors">
                {integration.icon}
              </div>
              <h3 className="text-white text-lg font-syne mb-2">
                {integration.name}
              </h3>
        ...
        <div className="grid md:grid-cols-[1fr,1fr] gap-24 items-start">
          ...
          <div>
            ...
            <motion.h2
              ...
              className="text-[54px] leading-[1.2] font-bold font-syne mt-6 text-[#2D2654]"
            >
              We Xero down food waste for{" "}
              <AnimatePresence mode="wait">
                <motion.span
                  key={words[currentWord]}
                  className="inline-block text-[#666ed2]"
